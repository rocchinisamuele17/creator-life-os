...
export function AppLayout() {
  ...
  useEffect(() => {
    ...
    if (bgStyle.backgroundImage) body.style.backgroundImage = bgStyle.backgroundImage;
    if (bgStyle.backgroundColor) body.style.backgroundColor = bgStyle.backgroundColor;
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "content": return <ContentMachine />;
      case "preview": return <PreviewStudio />;
      case "money": return <MoneyTracker />;
      case "life": return <VitaPersonale state={state} setState={setState} showToast={showToast} />;
      case "brands": return <BrandDeals state={state} setState={setState} showToast={showToast} />;
      case "settings": return <Settings state={state} setState={setState} showToast={showToast} />;
      case "admin": return <AdminDashboard />;
      case "ai": return <AICopilot showToast={showToast} />;
      default: return <Dashboard />;
    }
  };

  return (
    <div style={{ minHeight: "100vh", paddingBottom: "calc(20px + var(--safe-bottom))", position: "relative", overflowX: "hidden" }}>
      <div style={{ padding: "calc(12px + var(--safe-top)) 16px 0", borderBottom: "1px solid var(--glass-border)", background: "rgba(0,0,0,0.2)", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, flexWrap: "wrap", gap: 8 }}>
          <Header />
          {isConfigured && user && (
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div className="animate-glow" style={{ background: "var(--accent-gradient)", color: "#000", padding: "4px 8px", borderRadius: 12, fontSize: 10, fontWeight: 700 }}>
                🚀 {state.streak?.count || 0} gg
              </div>
              <button onClick={signOut} className="premium-btn secondary" style={{ padding: "4px 8px", fontSize: 10, minHeight: 32 }}>Esci</button>
            </div>
          )}
        </div>
      ...
      <div style={{ padding: "24px 16px", maxWidth: 1000, margin: "0 auto", width: "100%" }}>
